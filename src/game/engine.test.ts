...
describe("tick", () => {
  ...
  it("grows and scores when the head hits food", () => {
    ...
    expect(
      next.foods.some((food) => food.x === bait.x && food.y === bait.y),
    ).toBe(false);
...
describe("food spawn", () => {
  ...
  it("picks the same cell as a free-list scan", () => {
    const snake = [
      { x: 0, y: 0 },
      { x: 1, y: 0 },
    ];
    ...
    for (let seed = 1; seed <= 40; seed += 1) {
      ...
      for (const point of [...snake, ...existing]) {
      ...
      const listed = createRng(seed);
      const expected = free[listed.nextInt(free.length)];
      expect(spawnFood(snake, existing, createRng(seed))).toEqual(expected);
    }
  });
});
...
describe("engine replay", () => {
  ...
  it("stops counting once the replay dies mid-list", () => {
    const { state, recorded } = play(
      7,
      Array.from({ length: 80 }, () => "right"),
    );
    expect(state.status).toBe("gameover");
